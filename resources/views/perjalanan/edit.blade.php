...
<div class="max-w-xl mx-auto bg-white rounded-xl shadow-lg p-8">
    ...
    @endif
    <form action="{{ route('perjalanan.update', $perjalanan->id) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')
        <div>
            ...
            <select name="rencana_id" class="border border-primary-light rounded-lg w-full px-3 py-2 focus:ring-2 focus:ring-accent" required>
                ...
                @foreach($rencana as $r)
                    <option value="{{ $r->id }}" @if($perjalanan->rencana_id == $r->id) selected @endif>{{ $r->nama_rencana }}</option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <select name="peserta_id[]" class="border border-primary-light rounded-lg w-full px-3 py-2 focus:ring-2 focus:ring-accent" multiple required>
                @foreach($peserta as $p)
                    <option value="{{ $p->id }}" @if($perjalanan->peserta->contains($p->id)) selected @endif>{{ $p->nama }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="kegiatan" class="border border-primary-light rounded-lg w-full px-3 py-2 focus:ring-2 focus:ring-accent" value="{{ $perjalanan->kegiatan }}" required>
        </div>
        <div>
            <label class="block text-primary-dark font-semibold mb-1">Catatan</label>
            <textarea name="catatan" class="border border-primary-light rounded-lg w-full px-3 py-2 focus:ring-2 focus:ring-accent">{{ $perjalanan->catatan }}</textarea>
        </div>
        <div>
            ...
            <select name="status" class="border border-primary-light rounded-lg w-full px-3 py-2 focus:ring-2 focus:ring-accent">
                <option value="aktif" @if($perjalanan->status=='aktif') selected @endif>Aktif</option>
                <option value="selesai" @if($perjalanan->status=='selesai') selected @endif>Selesai</option>
            </select>
        </div>
